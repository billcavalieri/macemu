...
Branch **`arm64-jit`** at **`cf7f8eb4`** (S4 step 15, WP3 4b dispatcher +
verify). Past G5 (Finder / menu bar / About 9.2.1), interpreter baseline,
WP4 banks, WP3 4a integer-subset JIT, and WP3 4b fallback dispatcher
(identical boot, not faster; `NW_JIT=verify` 0-miss on the integer subset;
copy-out gated until `lwz`/`stw` helpers are vs-kpx'd).

**Next (strict order):** WP3 4b-2 (`lwz`/`stw` vs-kpx, then idle translate),
...
4b-2. **`lwz`/`stw` helpers vs-kpx**, then idle-loop translate; measure
    ...
    the load/store helpers match. WP3's "done" line: NK idle loop faster
    than the interpreter by a measured factor, 9.2.1 still page-faults correctly.
