 ...
 *  symlink to BasiliskII/src/SDL; this file lives here so G3 dirty-bbox
 *  / VideoPresent work does not touch Basilisk II. Live bedd28a3:
 *  PICSPIN_LEFT and CYCLE_LEFT with G2 HIT; WINDOW is unheld. Xcode
 *  Compile Sources point at this file. VOSF stays off. Do not mill 68k
 *  as G3.
 *
...
bool VideoInit(bool classic)
{
	...
	// Open display
	bool opened = monitor->video_open();
#ifdef SHEEPSHAVER
	if (opened && drv && sdl_window) {
		...
		nqd_dirty_bbox.x = 0;
		nqd_dirty_bbox.y = 0;
		nqd_dirty_bbox.w = VIDEO_MODE_X;
		nqd_dirty_bbox.h = VIDEO_MODE_Y;
		update_sdl_video(drv->s, 0, 0, VIDEO_MODE_X, VIDEO_MODE_Y);
#if NW_BOOT_LOG
		char buf[128];
		const char *mode_str = PrefsFindString("screen");
		snprintf(buf, sizeof(buf),
			 "G3: SDL2 window %dx%d depth=%d frameskip=%u screen=%s vosf=0",
			 VIDEO_MODE_X, VIDEO_MODE_Y, (int)VIDEO_MODE_DEPTH,
			 (unsigned)frame_skip,
			 mode_str ? mode_str : "default");
		nw_boot_log(buf);
		nw_boot_log(nw_boot_line_g3_sdl2_window());
#endif
		VideoPresent();
	} else {
#if NW_BOOT_LOG
		nw_boot_log("G3: SDL2 window missing");
#endif
	}
#endif
	return opened;
