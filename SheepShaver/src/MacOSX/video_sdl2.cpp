 ...
 *
 *  SheepShaver fork of the SDL2 video path. SheepShaver/src/SDL is a
 *  symlink to BasiliskII/src/SDL; this file lives under MacOSX so bbox /
 *  video_set_dirty_area do not touch Basilisk II. Xcode Compile Sources
 *  point here (not video_macosx.mm). VOSF stays off.
 *
...
#ifdef SHEEPSHAVER
	/*
	 * First present of the host SDL2 window (VOSF off). Marks the
	 * full framebuffer dirty so present_sdl_video is not a no-op
	 * on an empty union. Not G3: that still needs the installer on
	 * screen plus the live G2 HIT line.
	 */
	if (drv && drv->s) {
...
// After G2, NQD bbox is a hint not a clip. VOSF stays off.
#ifdef SHEEPSHAVER
static unsigned update_display_static_bbox(driver_base *drv)
#else
static void update_display_static_bbox(driver_base *drv)
#endif
{
	const VIDEO_MODE &mode = drv->mode;
#ifdef SHEEPSHAVER
	const bool blit = nw_video_need_blit(drv->s->pixels, the_buffer) != 0;
#else
	...
	uint32 x0 = 0, y0 = 0, x1 = VIDEO_MODE_X, y1 = VIDEO_MODE_Y;
#ifdef SHEEPSHAVER
	if (nqd_have_dirty) {
		if (!nw_video_full_scan(nw_guest_first_data_dsi_seen() ? 1 : 0)) {
			x0 = (uint32)nqd_dirty_bbox.x;
			y0 = (uint32)nqd_dirty_bbox.y;
			x1 = x0 + (uint32)nqd_dirty_bbox.w;
			y1 = y0 + (uint32)nqd_dirty_bbox.h;
			x0 &= ~(N_PIXELS - 1);
			y0 &= ~(N_PIXELS - 1);
			if (x1 > VIDEO_MODE_X)
				x1 = VIDEO_MODE_X;
		...
		nqd_have_dirty = false;
		nqd_dirty_bbox.x = nqd_dirty_bbox.y = 0;
		nqd_dirty_bbox.w = nqd_dirty_bbox.h = 0;
	}
#endif
	...
	for (uint32 y = y0; y < y1; y += N_PIXELS) {
		uint32 h = N_PIXELS;
		if (h > y1 - y)
			h = y1 - y;
		for (uint32 x = x0; x < x1; x += N_PIXELS) {
			uint32 w = N_PIXELS;
			if (w > x1 - x)
				w = x1 - x;
...
void video_set_dirty_area(int x, int y, int w, int h)
{
	...
	if (use_vosf && drv) {
		const VIDEO_MODE &mode = drv->mode;
		vosf_set_dirty_area(x, y, w, h, VIDEO_MODE_X, VIDEO_MODE_Y,
	...
	/* VOSF off: union into the static bbox tracker. Do not invent a second map. */
	if (!nw_video_clip_dirty(&x, &y, &w, &h, (int)VIDEO_MODE_X, (int)VIDEO_MODE_Y))
		return;
	SDL_Rect r;
	r.x = x;
	r.y = y;
